...
// const SUPABASE_URL = "https://wng..................rs.supabase.co";
// const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6.............E_0";
// const RECAPTCHA_SITE_KEY = "6Le3m6IrAAAAAHs.................";
...
export default function Contact({ t }: { t: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!token) {
        alert("reCAPTCHA non chargé ou invalide. Vérifie les domaines autorisés.");
        return;
      }

      // (Optionnel mais recommandé) stocker le token aussi
      const payload = { ...formData, recaptcha_token: token };

      const { error } = await supabase.from("portfolio-messages").insert([payload]);
      ...
      alert(err?.message ?? "Erreur inattendue (reCAPTCHA / réseau).");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <ReCAPTCHA
        ...
        size="invisible"
        asyncScriptOnLoad={() => {
          // optionnel: console.log("reCAPTCHA loaded");
        }}
      />
    </section>
  );
